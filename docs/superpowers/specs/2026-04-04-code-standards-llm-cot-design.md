...
Confirm which rules from which levels apply to this review.
...
For each applicable rule, walk through inspection steps:
...
Only then deliver your conclusion:
...
Review Scope:
- Rule Source: L1
- Specific Rules: [Rule 9] gorm fields use explicit tags, column only

Rule 9 Inspection:
- Check GORM model for index-related tags: found `gorm:"uniqueIndex"` tag
- Check ddl.sql for corresponding index definition: found no EMAIL unique index in ddl.sql
- Compare against standard: standard explicitly requires "indexes must and only managed via ddl.sql"

Conclusion: Non-Compliant
Basis: Rule 9, violates "gorm fields write column only, index tags forbidden"
Recommendation: Add EMAIL unique index definition in ddl.sql
```
